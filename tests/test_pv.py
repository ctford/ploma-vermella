...
def test_passive_matches_finds_a_real_passive():
    assert _passive_matches("The spec is generated by the agent.") == ["is generated"]
    assert _passive_matches("Checks were written before the code.") == ["were written"]
    assert _passive_matches("It is carefully maintained.") == ["is carefully maintained"]


def test_passive_matches_ignores_words_that_merely_end_in_en_or_ed():
    """These five were 9% of every hit in the book, and not one was a passive."""
    for phrase in ("it is often used by teams",       # 'often' is the adverb, not the verb
                   "the choice is between two loops",
                   "that is even harder",
                   "the interface is open",
                   "this is when it matters"):
        assert "is often" not in _passive_matches(phrase)
        assert not [m for m in _passive_matches(phrase)
                    if m.split()[-1] in {"often", "between", "even", "open", "when"}]


def test_passive_matches_still_flags_a_passive_after_a_skipped_word():
    """Skipping 'often' must not skip the passive it precedes."""
    assert _passive_matches("The spec is often regenerated overnight.") == \
        ["is often regenerated"]


def test_passive_matches_keeps_participial_adjectives():
